...
class AuthController {
    ...
    static async login(req, res) {
        ...
        try {
            ...
            if (!account) {
                return res.status(401).render("login", { title: "Đăng nhập", error: "Tài khoản không tồn tại" });
            }
            if (account.password !== password) {
                return res.status(401).render("login", { title: "Đăng nhập", error: "Mật khẩu không chính xác" });
            }
            
            const encodeData = { id: account.id, role: account.role, user_id: account.user_id }
